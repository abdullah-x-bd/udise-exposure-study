...
def main() -> None:
    if len(sys.argv) != 4:
        raise SystemExit("usage: combine_need_inspection_fastcodes.py PRIMARY_DIR CURRENT_DIR CORE_DIR")
    ...
    primary = models.loc[primary_mask].copy().sort_values("outcome")
    lines = [
        "# Need-to-inspection national experiment, final corrected inference",
        "",
        "Primary inference uses stable State-lineage clusters and memory-bounded alternating-projection absorption for school and district-by-year fixed effects.",
