...
p <- ggplot(d, aes(x = cohort, y = threshold_label)) +
  geom_tile(width = 0.92, height = 0.78, fill = "#F4F4F2", colour = "white", linewidth = 3) +
  ...
  labs(
    title = "THE FISCAL FINGERPRINT REPEATS",
    subtitle = "Four statutory thresholds. Four cohorts. The UDISE record repeatedly moves toward the new grant band.\nNumbers show the percentage-point jump in reaching the statutory target at the correctly aligned record.",
    x = "Assignment-enrolment cohort",
    y = NULL,
    caption = "Correct +3 reporting alignment. 250/251 is the primary design; the other thresholds are formula-fingerprint tests."
  ) +
  ...
  theme(
    ...
    axis.text.y = element_text(size = 13.2, face = "bold", colour = "#222222", lineheight = 1.05, margin = margin(r = 14)),
    ...
    plot.subtitle = element_text(size = 12.8, colour = "#222222", lineheight = 1.2, margin = margin(b = 22)),
    plot.caption = element_text(size = 9.5, colour = "#666666", hjust = 0, margin = margin(t = 18)),
    plot.margin = margin(30, 30, 28, 34)
  ) +
...
ggsave(file.path(out_dir, "csg_formula_fingerprint_R.png"), p,
       width = 12.8, height = 7.8, units = "in", dpi = 320, bg = "white")
ggsave(file.path(out_dir, "csg_formula_fingerprint_R.svg"), p,
       width = 12.8, height = 7.8, units = "in", bg = "white")
