...
def main():
    ...
    write_csv(out/'missingness_and_rates.csv',rows);write_csv(out/'state_missingness_and_rates.csv',states);(out/'validation.json').write_text(json.dumps({'assignment':adiag,'previous':prevdiag,'assignment_year':ay,'report_year':ry},indent=2,default=float),encoding='utf-8');print(json.dumps({'assignment_year':ay,'rows':len(rows),'state_rows':len(states)},indent=2))
