...
def main() -> None:
    ...
    new_base = "CREATE TEMP TABLE base AS SELECT CAST(p.{pid} AS VARCHAR) pseudocode,{nref(pc,'managment','p')} management,{nref(pc,'state','p')} state,{','.join(expr+' b_'+name for name,expr in base_out.items())} FROM {p1} p LEFT JOIN {fac} f ON CAST(p.{pid} AS VARCHAR)=CAST(f.{fid} AS VARCHAR)"
    ...
    new_sample = "CREATE TEMP TABLE sample AS SELECT e.pseudocode,e.enrol,b.state state,{num('m.'+qid(mgmt))} management_current,b.management management_base"
    ...
    # Store corrected results separately so the invalid one-state run cannot be mistaken for confirmatory output.
    code = code.replace("outputs/csg_focused_2022_2024", "outputs/csg_focused_2022_2024_corrected")
