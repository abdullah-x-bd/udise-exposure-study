...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="csg_pilot_") as td:
        ...
        con.execute(f"""
          ...
          GROUP BY 1
        """)
        # Discover current grant source. school_master_base contains grants_receipt in the established pipeline.
        ...
        q=f"""
          SELECT a.enrol, m.{grant} receipt{',m.'+state+' state' if state else ''}{',m.'+mgmt+' management' if mgmt else ''}
          FROM assignment a JOIN school_master_base m USING(pseudocode)
          WHERE a.enrol BETWEEN 1 AND 1300 AND m.{grant} IS NOT NULL
        """
        rows=con.execute(q).fetchall(); desc=[d[0] for d in con.description]
        # Cell means/counts and simple local-linear discontinuities.
        con.execute(f"CREATE TEMP TABLE sample AS {q}")
        cells=[]
        for cutoff,bw in ((30,20),(100,50),(250,100),(1000,250)):
            ...
            for e,n,mean,posmean,pr in cr: cells.append({'cutoff':cutoff,'enrol':e,'n':n,'mean_receipt':mean,'positive_mean':posmean,'positive_rate':pr})
        estimates=[]
        for cutoff,bws in {30:[10,15,20],100:[20,30,40,50],250:[30,50,75,100],1000:[100,150,250]}.items():
            for bw in bws:
                # pooled local linear with state FE through within-state demean in DuckDB regression via explicit state indicators is too costly;
                # pilot reports raw local linear and within-state bin means. Full study performs FE/clustered inference.
                r=con.execute(f"""
                  ...
                  stats AS (SELECT regr_slope(receipt,d) crude_diff FROM s)
                  SELECT COUNT(*), AVG(receipt) FILTER (WHERE d=0), AVG(receipt) FILTER (WHERE d=1), (SELECT crude_diff FROM stats) FROM s
                """).fetchone()
                # proper local-linear coefficients using normal equations in numpy
        ...
        print('N',len(rows),'management_field',mgmt)
        print('ESTIMATES')
        for r in estimates: print(json.dumps(r),flush=True)
        print('TOP GRANT VALUES')
        print(con.execute("SELECT receipt,COUNT(*) n FROM sample GROUP BY 1 ORDER BY n DESC LIMIT 30").fetchall())
        if mgmt:
            print('MANAGEMENT')
            print(con.execute(f"SELECT management,COUNT(*),AVG(receipt),AVG(CASE WHEN receipt>0 THEN 1.0 ELSE 0.0 END) FROM sample GROUP BY 1 ORDER BY 2 DESC").fetchall())
        con.close()
